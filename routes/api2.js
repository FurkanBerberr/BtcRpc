...
const getBlockFromHash = async (id) => {
    const apiUrl = `http://${USER}:${PASS}@192.168.1.76:5086`
...
router.get("/getblocks/:hash", async (req, res) => {
    let blockInfo = await getBlock(req.params.hash)
    let blockCounter = 0
    for (let i = 0; i < 5; i++) {
        let blockTxArray = blockInfo.result.tx
        //console.log("blockTxArray", blockTxArray)
        for (let j = 0; j < blockTxArray.length; j++) {
            const transaction = await getRawTransaction(blockTxArray[j])
            //console.log("transaction ", j, transaction.result.txid)
            let data = {
            ...
            //console.log("is coinbase ", Object.hasOwn(transaction.result.vin[0], "coinbase"))
            if (Object.hasOwn(transaction.result.vin[0], "coinbase")) {
                ...
                for (let k = 0; k < transaction.result.vin.length; k++) {
                    ...
                    if (Object.hasOwn(vin_transaction.result.vin[0], "coinbase")) {
                        ...
                        vin_info["value"] = vin_transaction.result.vout[vin_vout_value].value,
                            vin_info["address"] = vin_transaction.result.vout[vin_vout_value].scriptPubKey.address
                    }
                    data["vin"][k] = vin_info
                }
                for (let h = 0; h < transaction.result.vout.length; h++) {
                    let vout_info = {
                        ...
                        address: transaction.result.vout[h].scriptPubKey.address
                    }
